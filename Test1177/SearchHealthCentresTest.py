...
class SearchHealthCentresTest(unittest.TestCase):
    @classmethod
    def setUp(self):
    ...
    def test_search_for_health_centres(self):
        ...
        # accept cookies
        accept_cookie_button = self.driver.find_element_by_id(
        ...
        select_location_button = self.driver.find_element_by_class_name(
            "findcare-searchfield__autocomplete__item"
        )
        select_location_button.click()

        # search for chosen alternative
        search_care_button = self.driver.find_element_by_class_name(
